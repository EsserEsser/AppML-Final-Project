#!/usr/bin/env python3
"""
Crop all JPGs in a folder to 200x200 by removing the rightmost columns
that exceed a width of 200 pixels.

Images are expected to be 200x200, 201x200, or 202x200. Only those wider
than 200px are modified; everything else is left untouched. Cropping keeps
the leftmost 200 columns (i.e. the excess on the right is discarded).

Usage:
    python crop_to_200.py /path/to/folder
    python crop_to_200.py /path/to/folder --dry-run
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Crop JPGs to 200x200 by trimming the rightmost excess columns.")
