...
# ──────────────────────────────────────────────────────────────
def main():
    DATA_DIR    = r"Data\jpegs_all_300m"
    IMG_SIZE    = 600
    ...
    REGRESSOR_PTH = r"Benjamin\output\best_reg_300.pth"  # Set to .pth path to skip tuning and training
...
# ── 4. Pipeline Eval & Analytics ─────────────────────────────
    ...
    #Calculate matthew  correlation coefficient (MCC) for classifier
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "pipeline_pred_vs_actual_loglog_300.png"), dpi=150)
    plt.close()
    ...
    # ── Save per-tile validation results ──────────────────────
    results_path = os.path.join(OUTPUT_DIR, "validation_results_300.txt")
    with open(results_path, "w") as f:
        f.write("filename\tprediction\tprediction_rounded\ttrue_count\n")
        for i in range(len(ds_pipe_val)):
            fname = ds_pipe_val.samples[i][0].name
            pred  = max(all_preds_np[i], 0)
            f.write(f"{fname}\t{pred:.4f}\t{round(pred)}\t{int(all_targets_np[i])}\n")
    print(f"✓ Saved per-tile results to: {results_path} ({len(ds_pipe_val)} rows)")
