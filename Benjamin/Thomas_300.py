...
# ──────────────────────────────────────────────────────────────
def main():
    DATA_DIR    = r"C:\Users\Ben12\Documents\GitHub\AppML-Final-Project\Data\jpegs_all_300m"
    IMG_SIZE    = 600
    ...
    CLASSIFIER_PTH =  r"Benjamin\final_model_300.pth"# Set to .pth path to skip tuning and training
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "target_distribution_300.png"), dpi=150)
    plt.close()
    ...
    if RUN_ARCH_TUNING_R:
        def reg_objective(trial):
            ...
            try:
                ...
                print(f"Out of memory during trial: {e}. Pruning trial.")
                del m
    ...
    for epoch in pbar:
        ...
        if loss_v < best_r_mse:
            best_r_mse = loss_v
            pat = 0
            torch.save(regressor.state_dict(), os.path.join(OUTPUT_DIR, "best_reg_300.pth"))
        else:
            pat += 1
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "regressor_training_curves_300.png"), dpi=150)
    plt.close()
    
    
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "classifier_training_curves_300.png"), dpi=150)
    plt.close()


    regressor.load_state_dict(torch.load(os.path.join(OUTPUT_DIR, "best_reg_300.pth"), weights_only=False))
    ...
    dl_classifier_val = DataLoader(ds_pipe_val, batch_size=c_hp["batch_size"], shuffle=False, num_workers=NUM_WORKERS, pin_memory=False)
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, "classifier_confusion_matrix_300.png"), dpi=150)
    plt.close()
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, "regressor_pred_vs_actual_300.png"), dpi=150)
    plt.close()
    ...
    print("="*30)
    print("REGRESSOR (Truly Occupied Images Only):")
    print(f"  MSE: {reg_mse_true:.4f}")
    print(f"  MAE: {reg_mae_true:.4f}")
    print(f"  R² : {reg_r2_true:.4f}")
    print("-" * 30)
    print("REGRESSOR (Classifier-passed, may include false positives):")
    print(f"  MSE: {reg_mse_cls:.4f}")
    print(f"  MAE: {reg_mae_cls:.4f}")
    ...
    ax.legend(); ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "pipeline_pred_vs_actual_300.png"), dpi=150)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "pipeline_error_analysis_300.png"), dpi=150)
    plt.close()
